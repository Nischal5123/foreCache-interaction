...
```
```{r}
combined_model_full_same <- lmer(formula = probabsame~ Trial   + (1|Dataset) + (1|u), data = data_both_pall, REML = FALSE)
reducedmodel_same <- lmer(formula =  probabsame ~ 1   + (1|Dataset)+ (1|u), data = data_both_pall, REML = FALSE)
lrtest(reducedmodel_same, combined_model_full_same)
```
```{r}
combined_model_reporting_same <-lmer(formula = probabsame~Trial  + (1|Dataset) + (1|u), data = data_both_pall)
...
```
```{r}
combined_model_full_modify2 <- lmer(formula = probabmodify2~ Trial  + (1|Experiment) + (1|u), data = data_both_pall, REML = FALSE)
reducedmodel_modify2 <- lmer(formula =  probabmodify2 ~ 1  + (1|Experiment)  + (1|u), data = data_both_pall, REML = FALSE)
...
```{r}
combined_model_full_modify3 <- lmer(formula = probabmodify3~ Trial + (1|Experiment)+ (1|u), data = data_both_pall, REML = FALSE)
reducedmodel_modify3 <- lmer(formula =  probabmodify3 ~ 1 +  (1|Experiment)+(1|u), data = data_both_pall, REML = FALSE)
lrtest(reducedmodel_modify3, combined_model_full_modify3)
```
```{r}
combined_model_reporting_modify3 <-lmer(formula = probabmodify3~ Trial  + (1|Experiment) + (1|u), data = data_both_pall)
