...
class QLearningAgent:
    ...
    def train(self):
        ...
        for n_epi in range(50):
            ...
            while not done:
                ...
                for t in range(self.num_rollouts):
                    ...
                    target_q = r + self.gamma * next_q_values.max().item()

                    q_values_a = q_values[a].unsqueeze(0)  # Reshape q_values[a] to match target_q shape
                    loss = nn.MSELoss()(q_values_a, torch.tensor([target_q]))
        ...
        return np.mean(predictions), self.q_network# return last episodes accuracyas training accuracy
    def test(self,model):
        test_predictions = []
        ...
        for n_epi in range(1):
            ...
            while not done:
                    ...
                    if done:
                        break
        print("############ Test Accuracy :{},".format(np.mean(test_predictions)))
        return np.mean(test_predictions), split_accuracy
...
def run_experiment(user_list, algo, hyperparam_file):
    ...
    result_dataframe = pd.DataFrame(columns=['Algorithm', 'User', 'Threshold', 'LearningRate', 'Discount', 'Epsilon', 'Accuracy', 'StateAccuracy', 'Reward'])
    title = algo
