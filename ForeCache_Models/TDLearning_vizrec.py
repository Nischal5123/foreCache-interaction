    ...
    def test(self, Q, discount_factor, alpha, epsilon, threshold, num_episodes=1):
        ...
        for i_episode in range(1):
            ...
            for t in itertools.count():
                ...
                next_state, reward, done, prediction,true_reward, true_action, predicted_action, index = self.env.step(state, action, True)
                ...
                y_true.append((true_action, index, self.user, threshold))
                y_pred.append((predicted_action, index, self.user, threshold))
        ...
        return np.nanmean(stats),model_actions,split_accuracy, np.nanmean(reward_accumulated)/np.nanmean(reward_possible), y_true, y_pred
