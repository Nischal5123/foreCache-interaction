    ...
    def test(self, env, Q, discount_factor, alpha, epsilon,num_episodes=1,step_size=0.01):
        ...
        for i_episode in range(1):
            ...
            for i in stats:
                cnt += i
            cnt /= len(stats)
        return np.mean(stats), model_actions,split_accuracy
...
if __name__ == "__main__":
    ...
    p1 = multiprocessing.Process(target=obj2.hyper_param, args=(env,user_list_2D, 'qlearning',50,))
    #p3 = multiprocessing.Process(target=obj2.hyper_param,args=(env, user_list_first_time[:6], 'qlearning', 50,))
    #
    p2 = multiprocessing.Process(target=obj2.hyper_param, args=(env, user_list_2D, 'sarsa', 50,))
