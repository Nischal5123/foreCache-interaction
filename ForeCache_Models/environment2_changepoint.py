...
from ChangePoint import ChangePointDetection as cpd
...
class environment2:
    ...
    def reset(self, all=False, test=False):
        ...
        s, r, a, _ = self.cur_inter(self.steps)
        return s
    ...
    def process_data(self, filename, thres):
        ...
        """
        cp_obj= cpd(filename)
        df,_ = cp_obj.detect_change_points()
        ...
        for index, row in df.iterrows():
            ...
            change_point = row["IsChangePoint"]
            self.mem_cp.append(change_point)
    ...
    def cur_inter(self, steps):
        ...
        - (str): current action.
        """
        return self.mem_states[steps], self.mem_reward[steps], self.mem_action[steps], self.mem_cp[steps]
    ...
    def step(self, cur_state, act_arg, test):
        ...
        _, cur_reward, cur_action, cur_cpd = self.cur_inter(self.steps)

        # Peek at the next step to get the next state, reward, and action
        _, temp_step = self.peek_next_step()
        next_state, next_reward, next_action, _ = self.cur_inter(temp_step)
        ...
        return next_state, cur_reward, self.done, prediction, top_reward, cur_cpd
