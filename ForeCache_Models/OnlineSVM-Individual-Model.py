...
class OnlineSVM:
    ...
    def train(self, X_train, y_train):
        ...
        for i in range(len(X_train)):
            self.model.partial_fit([X_train[i]], [y_train[i]], classes=['same', 'modify-1', 'modify-2', 'modify-3'])
...
def run_experiment(user_list, dataset, task):
    ...
    for test_user_log in user_list:
        ...
        for thres in threshold:
            ...
            accuracy, granularPredictions, pred, ground_truth = model.evaluate(np.array(X_test), np.array(y_test))
    ...
    # Save results to CSV
    result_dataframe.to_csv(f"Experiments_Folder/Individual-Model/{dataset}/{task}/OnlineSVM-Individual-Model.csv", index=False)
    print(f"Dataset: {dataset} Task: {task} Algorithm: OnlineSVM-Individual-Model, Average Accuracy: {result_dataframe['Accuracy'].mean()}")
