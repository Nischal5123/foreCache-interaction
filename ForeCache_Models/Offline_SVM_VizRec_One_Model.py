...
class OfflineSVM:
    def __init__(self, max_iter=1000):
        """
        Initializes the OfflineSVM SVM model using SGDClassifier.
        """
...
def run_experiment(user_list, dataset, task):
    ...
    # Save results to CSV
    result_dataframe.to_csv(f"Experiments_Folder/VizRec/{dataset}/{task}/OfflineSVM-PartialTrain-SVM-Single-Model.csv", index=False)
...
def get_average_accuracy():
    """
    Get the average accuracy of the OfflineSVM SVM model.
    """
    ...
    for dataset in datasets:
        for task in tasks:
            result_df = pd.read_csv(f"Experiments_Folder/VizRec/{dataset}/{task}/OfflineSVM-PartialTrain-SVM-Single-Model.csv")
