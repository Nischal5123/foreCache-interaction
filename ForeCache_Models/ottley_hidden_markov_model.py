...
#        self.underlying_data[flatten_list(self.continuous_attributes)] = min_max_scaler.fit_transform(self.underlying_data[flatten_list(self.continuous_attributes)])
        ...
        self.bias_vector = 0.5 * np.ones(len(discrete_attributes))
        self.bias_sigma = 0.1
        ...
        attribute_names = [x if type(x) == str else '___'.join(x) for x in discrete_attributes]
        self.bias_column_names = [f'bias_{x}' for x in attribute_names]
        self.bias_over_time = pd.DataFrame(columns=self.bias_column_names)

        self.particles = pd.DataFrame(columns=discrete_attributes + self.bias_column_names)
    ...
    def update(self, observation_index):
        ...
        if np.sum(weights) == 0:
            resampled_indices = np.random.choice(self.num_particles, self.num_particles)
        else:
